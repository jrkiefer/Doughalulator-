...
- [x] **Post-launch tune-ups** · v1.0.1 full-record autosave (a 2 PM edit re-dirties and re-syncs the derived EON math) · v1.0.2 bold green SYNCED ✓ badge · v1.0.3 faster sync: 1 s debounce with an instant flush on leaving a field or tapping a choice, the two sheets sync in parallel, and both Apps Scripts write each row in one ranged write instead of cell-by-cell (owner must re-paste both scripts for the in-Google speedup) · v1.0.4 obvious date picker: ◀ ▶ day arrows and a visible "tap to pick any day" calendar affordance on the Active Date card.
