...
| | |
| --- | --- |
| **1.28.0** | The **By Size** table now shows its work. It used to end in one TRAYS column holding the plain bible plan, while the pills on the card above showed the trays after the batch rounding — so a night that rounded up read *LG 16* on one card and *LG 15* on the next, with nothing on screen to join them. That column is now three: **PLAN** (what the bible asked for), **ROUND** (what the batch rule added or trimmed to reach whole mixer runs — red when it adds, green when it takes away, 0 when it left that size alone), and **FINAL**, which is the highlighted coloured box and the number you actually make. FINAL always matches the pills above it now, because both cards read the same worked-out figures instead of each doing the sum themselves. |
