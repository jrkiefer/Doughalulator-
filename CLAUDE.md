...
- Apps Scripts (`apps-script/*/Code.gs`) are lock-guarded, validate terminally (bad date/empty/nonsense negatives — sales-left and temps may be negative), and match dates tolerantly (ISO, M/D/YYYY, 2-digit years). A secret-guarded doPost `{ type: 'wipe', confirm: 'WIPE ALL DATA' }` erases all data rows (headers, bible mirrors, and Dough Use formulas survive) — the app never sends it. Backend tests run the real `.gs` files in-process via `apps-script/harness.ts`, including a bible tripwire against `src/data`.
